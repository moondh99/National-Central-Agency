    ...
    try:
        ...
        if len(full_content) < 100:
            ...
            for line in lines:
                ...
                # 본문 끝 조건
                if (line.startswith('(사진=') or
                    line.startswith('Copyright') or
                    line.startswith('▶') or
                    'SBS' in line and ('제보' in line or '앱' in line) or
                    '무단 전재' in line or
                    'AI학습' in line):
                    break
...
def parse_sbs_rss_full_content(category='all'):
    ...
    category_urls = {
        ...
        'sports': 'https://news.sbs.co.kr/news/SectionRssFeed.do?sectionId=06&plink=RSSREADER',    # 스포츠
        'entertainment': 'https://news.sbs.co.kr/news/SectionRssFeed.do?sectionId=07&plink=RSSREADER'  # 연예
    }
    ...
    try:
        ...
        # CSV 파일로 저장
        if news_data:
            filename = f"SBS뉴스_{category}_전체본문_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("2. 특정 카테고리 뉴스")
        print("3. 다중 카테고리 뉴스")
        print("4. 빠른 테스트 (최신 뉴스 5개)")
        
        choice = input("\n선택 (1-4): ").strip()
        
        if choice == "1":
            print("\n🚀 전체 최신 뉴스 수집을 시작합니다...")
            result = parse_sbs_rss_full_content('all')
            
        elif choice == "2":
            print("\n📂 카테고리를 선택하세요:")
            categories = ['all', 'politics', 'economy', 'society', 'culture', 'international', 'sports', 'entertainment']
            korean_names = {
                'all': '전체 최신', 'politics': '정치', 'economy': '경제', 
                'society': '사회', 'culture': '문화', 'international': '국제',
                'sports': '스포츠', 'entertainment': '연예'
            }
            ...
            result = parse_sbs_rss_full_content(selected_category)
            
        elif choice == "3":
            print("\n📂 수집할 카테고리들을 선택하세요 (쉼표로 구분):")
            print("예: politics,economy,society")
            print("사용 가능: all, politics, economy, society, culture, international, sports, entertainment")
            ...
            result = scrape_sbs_multiple_categories(selected_categories)
            
        elif choice == "4":
            print("\n🚀 테스트 모드로 시작합니다...")
            # 테스트를 위해 최신 뉴스 몇 개만 처리
            result = parse_sbs_rss_full_content('all')
