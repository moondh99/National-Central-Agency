...
def extract_heraldjournal_article_content(url, rss_summary=""):
    ...
    try:
        ...
        # 본문 추출 - 헤럴드저널 HTML 구조에 맞게 수정
        content = ""
        
        # 방법 1: 헤럴드저널 기사 본문 구조 찾기
        content_selectors = [
            'div.article_content',       # 헤럴드저널 주요 기사 본문 클래스
        ...
        # 방법 2: P 태그 기반 추출 (헤럴드저널 특성에 맞게 수정)
        if len(content) < 200:
            ...
            for p in paragraphs:
                ...
                if (len(text) > 20 and 
                    not re.search(r'입력\s*\d{4}|수정\s*\d{4}|Copyright|저작권|헤럴드저널|heraldjournal', text) and
                    not text.startswith(('▶', '☞', '※', '■', '▲', '[', '※', '◆', '○', '△')) and
                    '@heraldjournal.co.kr' not in text and
...
def fetch_heraldjournal_rss_to_csv(rss_url, output_file, max_articles=30):
    """헤럴드저널 RSS를 파싱하여 CSV로 저장"""
    ...
    with open(output_file, 'w', newline='', encoding='utf-8-sig') as csvfile:
        ...
        for i, entry in enumerate(feed.entries[:max_articles]):
            try:
                ...
                # URL에서 카테고리 추출 시도 (헤럴드저널 URL 구조 기반)
                if not category:
                    url_category_map = {
                        'allArticle': '전체기사',
                        'clickTop': '인기기사',
                ...
                # 기사 본문 및 기자명 추출
                reporter, content = extract_heraldjournal_article_content(link, summary)
...
if __name__ == "__main__":
    # 헤럴드저널 RSS URL 옵션들 (첨부된 이미지 기반)
    heraldjournal_rss_options = {
        "전체기사": "http://www.heraldjournal.co.kr/rss/allArticle.xml",
        "인기기사": "http://www.heraldjournal.co.kr/rss/clickTop.xml",
        "뉴스": "http://www.heraldjournal.co.kr/rss/S1N1.xml",
        "기획특집": "http://www.heraldjournal.co.kr/rss/S1N2.xml",
        "오피니언": "http://www.heraldjournal.co.kr/rss/S1N3.xml",
        "포토영상": "http://www.heraldjournal.co.kr/rss/S1N4.xml",
        "헤럴드톡스": "http://www.heraldjournal.co.kr/rss/S1N5.xml"
    }
    
    # 원하는 카테고리 선택
    print("헤럴드저널 RSS 수집기")
    print("="*50)
    for key in heraldjournal_rss_options.keys():
        print(f"- {key}")
    
    # 카테고리 입력 받기
    selected_category = input("\n수집할 카테고리를 입력하세요 (기본값: 전체기사): ").strip()
    if not selected_category or selected_category not in heraldjournal_rss_options:
    ...
    selected_rss = heraldjournal_rss_options[selected_category]
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    output_file = f"results/heraldjournal_{selected_category}_{timestamp}.csv"
    
    print(f"\n🚀 {selected_category} 카테고리에서 {max_articles}개 기사 수집 시작!")
    print(f"📁 저장 파일: {output_file}\n")
    
    # 실행
    fetch_heraldjournal_rss_to_csv(selected_rss, output_file, max_articles)
