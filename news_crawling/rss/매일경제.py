...
class MaeilKyungjaeCrawler:
    def __init__(self):
        # 로깅 설정
        logging.basicConfig(
            level=logging.INFO,
            format="%(asctime)s - %(levelname)s - %(message)s",
            handlers=[logging.FileHandler("maeil_kyungjae_rss_crawler.log", encoding="utf-8"), logging.StreamHandler()],
        )
        self.logger = logging.getLogger(__name__)

        # 검증된 RSS 피드 목록 (실제 작동하는 URL만)
        self.rss_feeds = {
            "헤드라인": "https://www.mk.co.kr/rss/30000001/",
            "전체뉴스": "https://www.mk.co.kr/rss/40300001/",
            "경제": "https://www.mk.co.kr/rss/30100041/",
            "증권": "https://www.mk.co.kr/rss/50000001/",
            "IT과학": "https://www.mk.co.kr/rss/40200003/",
        }
    ...
    def get_article_content(self, url):
        ...
        try:
            ...
            # 매일경제 기사 본문 셀렉터들
            content_selectors = [
                "html > body > div:nth-of-type(4) > main > section:nth-of-type(1) > div:nth-of-type(3) > section > div:nth-of-type(1) > div:nth-of-type(1) > div:nth-of-type(1)",
    ...
    def crawl_all_feeds(self):
        """모든 RSS 피드 크롤링"""
        self.logger.info("매일경제 RSS 크롤링 시작")
    ...
    def save_to_csv(self, filename=None):
        """CSV 파일로 저장"""
        # 파일명 설정: results/언론사명_전체_{timestamp}.csv
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"results/{self.media_name}_전체_{timestamp}.csv"

        try:
            # rows 데이터 생성: 언론사, 제목, 날짜, 카테고리, 기자명, 본문
            rows = []
...
def main():
    """메인 실행 함수"""
    crawler = MaeilKyungjaeCrawler()

    print("🚀 매일경제 RSS 크롤링을 시작합니다...")
    print("📰 수집 대상 카테고리: 헤드라인, 전체뉴스, 경제, 증권, IT과학")
