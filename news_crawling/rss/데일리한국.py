...
def extract_hankooki_article_content(url, rss_summary=""):
    ...
    try:
        ...
        # 기자명 추출 - 데일리한국 패턴에 맞게 수정
        reporter = ""
        reporter_patterns = [
            r'([가-힣]{2,4})\s*기자\s*([a-zA-Z0-9_.+-]+@hankooki\.com)',     # 기자명 기자 이메일@hankooki.com
            r'([가-힣]{2,4})\s*기자\s*[a-zA-Z0-9_.+-]+@hankooki\.com',        # 기자명 기자 이메일
        ...
        for pattern in reporter_patterns:
            ...
            if match:
                ...
                reporter = re.sub(r'기자|특파원|편집위원|팀장|취재|글|선임기자|수석기자', '', reporter).strip()
        ...
        if len(content) < 200:
            ...
            for p in paragraphs:
                ...
                if (len(text) > 20 and 
                    not re.search(r'입력\s*\d{4}|수정\s*\d{4}|Copyright|저작권|데일리한국|hankooki', text) and
                    not text.startswith(('▶', '☞', '※', '■', '▲', '[', '※', '◆', '○', '△')) and
                    '@hankooki.com' not in text and
...
if __name__ == "__main__":
    ...
    print("데일리한국 RSS 수집기")
    print("="*50)
    for key in hankooki_rss_options.keys():
        print(f"- {key}")
    
    # 카테고리 입력 받기
    selected_category = input("\n수집할 카테고리를 입력하세요 (기본값: 전체기사): ").strip()
    if not selected_category or selected_category not in hankooki_rss_options:
    ...
    selected_rss = hankooki_rss_options[selected_category]
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    output_file = f"results/hankooki_{selected_category}_{timestamp}.csv"
    
    print(f"\n🚀 {selected_category} 카테고리에서 {max_articles}개 기사 수집 시작!")
    print(f"📁 저장 파일: {output_file}\n")
    
    # 실행
    fetch_hankooki_rss_to_csv(selected_rss, output_file, max_articles)
