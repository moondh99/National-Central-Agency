...
import time
import random
import os  # import 추가 for 결과 디렉토리 생성
...
def extract_jbnews_article_content(url, rss_summary=""):
    ...
    try:
        ...
        # 본문 추출 - 원문 페이지의 article-body 우선 추출
        content = ""
        ...
        if len(content) < 200:
            ...
            for p in paragraphs:
                ...
                if (
                    len(text) > 20
                    and not re.search(r"입력\s*\d{4}|수정\s*\d{4}|Copyright|저작권|중부매일|jbnews", text)
                    and not text.startswith(("▶", "☞", "※", "■", "▲", "[", "※", "◆", "○", "△"))
                    and "@jbnews.com" not in text
...
if __name__ == "__main__":
    jbnews_rss_options = {"전체기사": "http://www.jbnews.com/rss/allArticle.xml"}
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    output_dir = "results"
    os.makedirs(output_dir, exist_ok=True)
    output_file = f"{output_dir}/중부매일_전체_{timestamp}.csv"
    ...
    with open(output_file, "w", newline="", encoding="utf-8-sig") as csvfile:
        ...
        for category, rss_url in jbnews_rss_options.items():
            print(f"\n📡 {category} 카테고리 수집 중...")
            try:
                headers = {"User-Agent": get_random_user_agent()}
                response = requests.get(rss_url, headers=headers, timeout=10)
            ...
            for entry in feed.entries[:20]:
                ...
                if hasattr(entry, "author"):
                    reporter = re.sub(r"\s*기자\s*", "", entry.author.strip())
                # 본문 추출
                print(f"    {title[:30]}...")
                _, content = extract_jbnews_article_content(entry.link)
                if not reporter:
                    reporter = "미상"
                if len(content.strip()) < 20:
                    print("     ⚠ 본문 짧아 스킵")
