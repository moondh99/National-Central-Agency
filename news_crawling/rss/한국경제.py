...
def extract_hankyung_article_content(url, rss_summary=""):
    ...
    try:
        ...
        # 본문 추출 - 한국경제 HTML 구조에 맞게 수정
        content = ""
        
        # 방법 1: 한국경제 기사 본문 구조 찾기
        content_selectors = [
            'div.article-body',          # 한국경제 주요 기사 본문 클래스
            'div[class*="article"]',     # article 관련 클래스
            'div[class*="content"]',     # content 관련 클래스
            'div[class*="news"]',        # news 관련 클래스
            'div.wrap_cont',             # 한국경제 컨텐츠 래퍼
            'article',                   # article 태그
            'main',                      # main 태그
        ...
        # 방법 2: P 태그 기반 추출 (한국경제 특성에 맞게 수정)
        if len(content) < 200:
            ...
            for p in paragraphs:
                ...
                if (len(text) > 20 and 
                    not re.search(r'입력\s*\d{4}|수정\s*\d{4}|Copyright|저작권|한국경제|hankyung', text) and
                    not text.startswith(('▶', '☞', '※', '■', '▲', '[', '※', '◆', '○')) and
                    '@hankyung.com' not in text and
...
def fetch_hankyung_rss_to_csv(rss_url, output_file, max_articles=30):
    """한국경제 RSS를 파싱하여 CSV로 저장"""
    ...
    with open(output_file, 'w', newline='', encoding='utf-8-sig') as csvfile:
        ...
        for i, entry in enumerate(feed.entries[:max_articles]):
            try:
                ...
                # 카테고리 추출 (한국경제는 RSS에서 카테고리 정보가 제한적일 수 있음)
                category = ""
                ...
                # 기사 본문 및 기자명 추출
                reporter, content = extract_hankyung_article_content(link, summary)
...
if __name__ == "__main__":
    ...
    print("한국경제 RSS 수집기")
    print("="*50)
    for key, value in hankyung_rss_options.items():
        print(f"- {key}")
    
    # 카테고리 입력 받기
    selected_category = input("\n수집할 카테고리를 입력하세요 (기본값: 전체뉴스): ").strip()
    if not selected_category or selected_category not in hankyung_rss_options:
    ...
    selected_rss = hankyung_rss_options[selected_category]
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    output_file = f"results/hankyung_{selected_category}_{timestamp}.csv"
    
    print(f"\n🚀 {selected_category} 카테고리에서 {max_articles}개 기사 수집 시작!")
    print(f"📁 저장 파일: {output_file}\n")
    
    # 실행
    fetch_hankyung_rss_to_csv(selected_rss, output_file, max_articles)
