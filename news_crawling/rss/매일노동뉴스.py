...
def extract_labortoday_article_content(url, rss_summary=""):
    ...
    try:
        ...
        content = ""
        # 원문 페이지의 article 요소에서 본문 추출
        fixed_elem = soup.select_one("article#article-view-content-div")
        if fixed_elem:
            content = fixed_elem.get_text().strip()
            print(f"    고정 위치 본문 길이: {len(content)}")
            return reporter, content
        # 고정 위치 본문이 없으면 원문 페이지에서만 추출하므로 빈 문자열 반환
        return reporter, ""
        ...
        if len(content) < 200:
            ...
            for p in paragraphs:
                ...
                if (
                    len(text) > 20
                    and not re.search(r"입력\s*\d{4}\.\d{2}\.\d{2}|저작권자|매일노동뉴스|무단전재|재배포 금지", text)
                    and not text.startswith(("▶", "☞", "※", "■", "▲", "[", "SNS"))
                    and "@labortoday.co.kr" not in text
                    and "SNS 기사보내기" not in text
                ):
        ...
        if len(content) < 100:
            ...
            for line in lines:
                ...
                # 기사 시작 감지
                if not article_started and ("기자명" in line or "입력 20" in line or len(line) > 50):
...
def fetch_labortoday_rss_to_csv(rss_url, max_articles=30):
    """매일노동뉴스 RSS를 파싱하여 기사 목록으로 반환"""
    ...
    for i, entry in enumerate(feed.entries[:max_articles]):
        try:
            ...
            if hasattr(entry, "author") and entry.author:
                ...
                # '기자' 접미어 제거
                rss_reporter = re.sub(r"\s*기자$", "", rss_reporter)
    ...
    print(f"\n{'='*70}")
    print(f"🎉 완료! 총 성공 기사: {success_count}/{total_count}개 ({success_count/total_count*100:.1f}%)")
    print(f"{'='*70}")
    return rows


# 자동 실행: 언론사별 통합 CSV 생성
if __name__ == "__main__":
    # 매일노동뉴스 RSS URL 옵션들
    labortoday_rss_options = {"전체기사": "https://www.labortoday.co.kr/rss/allArticle.xml"}
    max_articles = 20
    all_rows = []
    for category, rss_url in labortoday_rss_options.items():
        print(f"\n🚀 {category} 카테고리에서 {max_articles}개 기사 수집 시작!")
        rows = fetch_labortoday_rss_to_csv(rss_url, max_articles)
        for row in rows or []:
            # 언론사와 카테고리 필드 추가
            row["언론사"] = "매일노동뉴스"
            row["카테고리"] = category
    ...
    if all_rows:
        ...
        output_file = f"results/매일노동뉴스_전체_{timestamp}.csv"
