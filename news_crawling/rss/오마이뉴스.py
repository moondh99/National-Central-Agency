...
def extract_ohmynews_article_content(url):
    ...
    try:
        ...
        for p in paragraphs:
            ...
            if (
                len(text) > 20
                and not re.search(r"copyright|저작권|오마이뉴스|구독|댓글|공유", text, re.IGNORECASE)
                and not text.startswith(("▶", "☞", "※", "■", "▲", "[", "●", "관련기사"))
                and "기자" not in text
                or len(text) > 50
            ):  # 기자명만 있는 짧은 문단 제외
        ...
        if len(content) < 100:
            ...
            for line in lines:
                ...
                if (
                    len(line) > 30
                    and not re.search(r"copyright|저작권|오마이뉴스|구독|댓글|공유|관련기사", line, re.IGNORECASE)
...
if __name__ == "__main__":
    # 오마이뉴스 RSS URL
    rss_url = "https://rss.ohmynews.com/rss/ohmynews.xml"
    max_articles = 20
    rows = []
    feed = feedparser.parse(rss_url)
    ...
    for entry in entries:
        ...
        # 기자명
        reporter = entry.author.strip() if hasattr(entry, "author") and entry.author else ""
        # 본문 (description HTML)
        desc_html = entry.description if hasattr(entry, "description") else ""
        content = BeautifulSoup(desc_html, "html.parser").get_text().strip()
        rows.append(
            {
                "언론사": "오마이뉴스",
                "제목": title,
    ...
    filename = f"results/오마이뉴스_전체_{timestamp}.csv"
