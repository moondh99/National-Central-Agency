    ...
    try:
        ...
        for pattern in reporter_patterns:
            ...
            if match:
                ...
                reporter = re.sub(r'기자|특파원|편집위원|팀장|취재|글|선임기자|수석기자|논설위원|컨슈머타임스', '', reporter).strip()
        ...
        if len(content) < 200:
            ...
            for p in paragraphs:
                ...
                if (len(text) > 20 and 
                    not re.search(r'입력\s*\d{4}|수정\s*\d{4}|Copyright|저작권|컨슈머타임스|cstimes', text) and
                    not text.startswith(('▶', '☞', '※', '■', '▲', '[', '※', '◆', '○', '△')) and
                    '@cstimes.com' not in text and
...
if __name__ == "__main__":
    # 컨슈머타임스 RSS URL 옵션들 (첨부된 이미지 기반)
    cstimes_rss_options = {
        "전체기사": "https://www.cstimes.com/rss/allArticle.xml",
        "인기기사": "https://www.cstimes.com/rss/clickTop.xml",
        "파이낸셜컨슈머": "https://www.cstimes.com/rss/S1N2.xml",
        "산업": "https://www.cstimes.com/rss/S1N3.xml",
        "뉴스": "https://www.cstimes.com/rss/S1N4.xml",
        "오피니언": "https://www.cstimes.com/rss/S1N6.xml",
        "사람들": "https://www.cstimes.com/rss/S1N7.xml",
        "컨슈머리뷰": "https://www.cstimes.com/rss/S1N8.xml",
        "Public": "https://www.cstimes.com/rss/S1N12.xml"
    }
    
    # 원하는 카테고리 선택
    print("컨슈머타임스 RSS 수집기 (소비자 전문)")
    print("="*50)
    for key in cstimes_rss_options.keys():
        print(f"- {key}")
    
    # 카테고리 입력 받기
    selected_category = input("\n수집할 카테고리를 입력하세요 (기본값: 전체기사): ").strip()
    if not selected_category or selected_category not in cstimes_rss_options:
    ...
    selected_rss = cstimes_rss_options[selected_category]
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    output_file = f"results/cstimes_{selected_category}_{timestamp}.csv"
    
    print(f"\n🚀 {selected_category} 카테고리에서 {max_articles}개 기사 수집 시작!")
    print(f"📁 저장 파일: {output_file}\n")
    
    # 실행
    fetch_cstimes_rss_to_csv(selected_rss, output_file, max_articles)
