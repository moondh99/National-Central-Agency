...
def extract_kado_article_content(url, rss_summary=""):
    """강원도민일보 기사 URL에서 본문과 기자명을 추출"""
    try:
        ...
        # 기자명 추출 - 강원도민일보 패턴에 맞게 수정
        reporter = ""
        reporter_patterns = [
            r'([가-힣]{2,4})\s*기자\s*([a-zA-Z0-9_.+-]+@kado\.net)',             # 기자명 기자 이메일@kado.net
            r'([가-힣]{2,4})\s*기자\s*[a-zA-Z0-9_.+-]+@kado\.net',                # 기자명 기자 이메일
        ...
        for pattern in reporter_patterns:
            ...
            if match:
                ...
                reporter = re.sub(r'기자|특파원|편집위원|팀장|취재|글|선임기자|수석기자|지사장|지국장|강원도민일보', '', reporter).strip()
                if len(reporter) >= 2 and len(reporter) <= 4:
                    break
        
        # 본문 추출 - 강원도민일보 HTML 구조에 맞게 수정
        content = ""
        
        # 방법 1: 강원도민일보 기사 본문 구조 찾기
        content_selectors = [
            'div.article_content',       # 강원도민일보 주요 기사 본문 클래스
        ...
        # 방법 2: P 태그 기반 추출 (강원도민일보 특성에 맞게 수정)
        if len(content) < 200:
            ...
            for p in paragraphs:
                ...
                if (len(text) > 20 and 
                    not re.search(r'입력\s*\d{4}|수정\s*\d{4}|Copyright|저작권|강원도민일보|kado', text) and
                    not text.startswith(('▶', '☞', '※', '■', '▲', '[', '※', '◆', '○', '△')) and
                    '@kado.net' not in text and
...
def fetch_kado_rss_to_csv(rss_url, output_file, max_articles=30):
    """강원도민일보 RSS를 파싱하여 CSV로 저장"""
    ...
    with open(output_file, 'w', newline='', encoding='utf-8-sig') as csvfile:
        ...
        for i, entry in enumerate(feed.entries[:max_articles]):
            try:
                ...
                # URL에서 카테고리 추출 시도 (강원도민일보 URL 구조 기반)
                if not category:
                    url_category_map = {
                        'allArticle.xml': '전체기사',
                        'clickTop.xml': '인기기사',
                ...
                # 기사 본문 및 기자명 추출
                reporter, content = extract_kado_article_content(link, summary)
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("주요 카테고리:")
    main_categories = ["전체기사", "인기기사", "지역", "정치", "경제", "사회"]
    for key in main_categories:
        if key in kado_rss_options:
            print(f"- {key}")
    
    print("\n전체 카테고리:")
    for key in kado_rss_options.keys():
        print(f"- {key}")
    
    # 카테고리 입력 받기
    selected_category = input("\n수집할 카테고리를 입력하세요 (기본값: 전체기사): ").strip()
    if not selected_category or selected_category not in kado_rss_options:
    ...
    selected_rss = kado_rss_options[selected_category]
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    output_file = f"results/kado_{selected_category}_{timestamp}.csv"
    
    print(f"\n🚀 {selected_category} 카테고리에서 {max_articles}개 기사 수집 시작!")
    print(f"📁 저장 파일: {output_file}\n")
    
    # 실행
    fetch_kado_rss_to_csv(selected_rss, output_file, max_articles)
