...
def extract_article_content(url):
    ...
    try:
        ...
        # 댓글, 공유 버튼 등 제거
        for unwanted_class in soup.find_all(class_=re.compile(r'comment|share|social|related|recommend|bookmark|print')):
...
if __name__ == "__main__":
    ...
    # 사용할 RSS 선택
    selected_rss = rss_options["전체"]  # 원하는 카테고리로 변경
    
    # 파일명에 현재 시간 추가
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    output_file = f"results/yonhap_news_{timestamp}.csv"
    
    # 실행
    fetch_yonhap_rss_to_csv(selected_rss, output_file, max_articles=30)
