    ...
    try:
        ...
        if len(content) < 200:
            ...
            for p in paragraphs:
                ...
                if (len(text) > 20 and 
                    not re.search(r'입력\s*\d{4}\.\d{2}\.\d{2}|Copyright|저작권|서울자치신문|www\.onseoul\.net', text) and
                    not text.startswith(('▶', '☞', '※', '■', '▲', '[')) and
                    not re.search(r'[가-힣]{2,4}\s*기자\s*\d{4}-\d{2}-\d{2}', text)):
        ...
        if len(content) < 100:
            ...
            for line in lines:
                ...
                if not article_started and (
                    len(line) > 30 and 
                    not line.startswith(('서명옥', '김정민', '박현수')) and
                    not re.search(r'\.(png|jpg|jpeg)$', line)):
                    article_started = True
                
                # 기사 본문 수집
                if article_started and len(line) > 10:
                    # 불필요한 라인 제외
                    if not re.search(r'Copyright|저작권|서울자치신문|www\.onseoul\.net|[가-힣]{2,4}\s*기자\s*\d{4}-\d{2}-\d{2}', line):
...
def fetch_onseoul_rss_to_csv(rss_url, output_file, max_articles=30):
    ...
    with open(output_file, 'w', newline='', encoding='utf-8-sig') as csvfile:
        ...
        for i, entry in enumerate(feed.entries[:max_articles]):
            try:
                ...
                # 기사 본문 및 기자명 추출
                reporter, content = extract_onseoul_article_content(link, summary)
...
if __name__ == "__main__":
    # 서울자치신문 RSS URL 옵션들
    onseoul_rss_options = {
        "전체기사": "https://www.onseoul.net/rss/allArticle.xml",
        "인기기사": "https://www.onseoul.net/rss/clickTop.xml",
        "서울시뉴스": "https://www.onseoul.net/rss/SIN1.xml",
        "오피니언": "https://www.onseoul.net/rss/SIN2.xml",
        "포토": "https://www.onseoul.net/rss/SIP1.xml",
        "기획특집": "https://www.onseoul.net/rss/SIN11.xml",
        "문화": "https://www.onseoul.net/rss/SIN13.xml",
        "서울자치신문TV": "https://www.onseoul.net/rss/SIN21.xml",
        "정치": "https://www.onseoul.net/rss/SIN22.xml",
        "사회": "https://www.onseoul.net/rss/SIN23.xml",
        "경제": "https://www.onseoul.net/rss/SIN24.xml",
        "공연": "https://www.onseoul.net/rss/S2N83.xml",
        "TV영화": "https://www.onseoul.net/rss/S2N86.xml",
        "강남": "https://www.onseoul.net/rss/gangnam.xml"
    }
    
    # 원하는 카테고리 선택
    print("서울자치신문 RSS 수집기")
    print("="*50)
    for key, value in onseoul_rss_options.items():
        print(f"- {key}")
    
    # 카테고리 입력 받기
    selected_category = input("\n수집할 카테고리를 입력하세요 (기본값: 전체기사): ").strip()
    if not selected_category or selected_category not in onseoul_rss_options:
    ...
    selected_rss = onseoul_rss_options[selected_category]
    timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    output_file = f"results/onseoul_{selected_category}_{timestamp}.csv"
    
    print(f"\n🚀 {selected_category} 카테고리에서 {max_articles}개 기사 수집 시작!")
    print(f"📁 저장 파일: {output_file}\n")
    
    # 실행
    fetch_onseoul_rss_to_csv(selected_rss, output_file, max_articles)
