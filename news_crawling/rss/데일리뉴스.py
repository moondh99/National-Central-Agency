...
class DailyNewsCrawler:
    def __init__(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('daily_news_rss_crawler.log', encoding='utf-8'),
        ...
        # 데일리뉴스 RSS 피드 목록
        self.rss_feeds = {
            "전체기사": "https://www.idailynews.co.kr/rss/allArticle.xml",
            "인기기사": "https://www.idailynews.co.kr/rss/clickTop.xml"
        }
        ...
        try:
            ...
            for i, item in enumerate(items[:8]):
                try:
                    ...
                    # 먼저 RSS description 사용
                    self.logger.info(f"기사 처리 중: {title[:30]}...")
                    content = self.get_content_from_description(description)
                    
                    # RSS description이 너무 짧거나 없는 경우 본문 추출 시도
                    if content in ["RSS description 없음", "RSS description이 너무 짧음"]:
                        self.logger.info(f"본문 추출 시도: {title[:30]}...")
    ...
    def save_to_csv(self, filename=None):
        ...
        if not filename:
            ...
            filename = f"results/daily_news_articles_{timestamp}.csv"
